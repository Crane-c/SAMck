...
'''
def calc_loss(outputs, label_batch, ce_loss, dice_loss, edge_weight=None, dice_weight=0.8):
    logits = outputs['low_res_logits']
    labels = label_batch.long()
    
    # 边缘权重增强（可选）
    if edge_weight is not None:
        ce_loss = (ce_loss(logits, labels) * edge_weight).mean()
    else:
        ce_loss = ce_loss(logits, labels)
    
    # 使用Generalized Dice Loss
    dice_loss = dice_loss(logits, labels, softmax=True)
    
    # 动态权重调整（示例：按训练进度调整）
    current_dice_weight = dice_weight * (1 - epoch / max_epochs)  # 线性衰减
    loss = (1 - current_dice_weight) * loss_ce + current_dice_weight * loss_dice
    
    return loss, loss_ce, loss_dice
'''

def trainer_rock(args, model, snapshot_path, multimask_output, low_res):
    from datasets.dataset_rock import Rock_dataset, RandomGenerator
    ...
    db_train = Rock_dataset(base_dir=args.root_path, list_dir=args.list_dir, split="train",
